import { EventEmmiter, AppEvent } from "../../libs/EventEmmiter"
import { Inline } from "../../libs/betterInline"
...
class EditorStyleManager {
    ...
    set envSelector(vl: string) {
        if (vl && !vl.endsWith(" ")) vl = vl + " "
        this._envSelector = vl
    }
    
    get envSelector(): string {
    ...
    add(selector: string, styles: CSSStyles, option: Record<string, any> = {}) {
        ...
        for (let key in styles) {
            if (key === "@extend") {
                let rule: CSSStyleRule | null | Record<string, any> = this.find(styles[key])
                if (!rule) continue
    ...
    remove(selector: string | number, sheet = this.sheet): boolean {
    ...
    find(selector: string, sheet = this.sheet) {
...
type EditorState = {
    isFakeRange: boolean,
    focusedBlock: Record<string, any> | null,
} & Record<string, any>
...
export class StackEditor extends EventEmmiter {
    ...
    cont!: HTMLElement
    content!: HTMLElement
    blocksBox!: HTMLElement
    btnsBox!: HTMLElement

    ui: EditorStyleManager = new EditorStyleManager()
    inline: Inline = new Inline()

    comps: Record<string, Function> = {}

    state: EditorState = {
        isFakeRange: false,
        focusedBlock: null,
    }
    blocksMeta: WeakMap<HTMLElement, Record<string, any>> = new WeakMap()
    ...
    async init() {
        await this.initCore()
        if(this.emit("init").prevented) return
        this.initStyles()
        this.initComponents()
    ...
    async initCore() {
        ...
        for (const toolName in this.op.coreTools) {
            ...
            try {
                ...
                if (typeof tool.init !== "function") continue
                let res = tool.init()
    ...
    initComponents() {
        this.comps.input = (config: Record<string, any> = {}) => {
            const conf = { ...{
                trimInp: true,
                placeholder: "",
                placeholderType: "",
    ...
    initProxyBlocks() {
        const getBlocks = (cont: HTMLElement) => {
        ...
        const findBlock = (e: MouseEvent, workspace: HTMLElement = this.blocksBox): boolean => {
            ...
            for (const block of blocks) {
                ...
                if (rect.top <= top && rect.top + rect.height >= top) {
                    let workspaces = Array.from(block.querySelectorAll(".elib-blocks")) as Array<HTMLElement>
        ...
        this.blocksBox.addEventListener("focusin", (e) => {
            ...
            const block = target.closest(".elib-block") as HTMLElement | null
            if (!block) return
    ...
    async initMenuBtns() {
        ...
        let lastBlock: Record<string, any> | null = null
        box.addEventListener("touchstart", () => {
            if (window.innerWidth < 768) lastBlock = this.state.focusedBlock
        })
        ...
        plusBtn.addEventListener("click", () => {
            ...
            if (this.cont.querySelector('.elib-newBlockMenu')) return
            this.newBlockMenu()
        })
        ...
        const place = async (block: HTMLElement) => {
        ...
        this.on("focusBlocks", (e: AppEvent) => {
    ...
    initPlugins() {
        ...
        const proceed = (toolName: string, obj: Record<string, any>, type?: string) => {
        ...
        for (const className in this.op.blockTools) {
            ...
            if (!Array.isArray(conf.options)) continue
            ...
            for (const plugins of conf.options) {
                ...
                for (const pluginName in plugins) {
                    ...
                    let btn = document.createElement("button") as HTMLElement
                    btn.setAttribute("data-tool", className)
                ...
                if (btns.length) this.btns.blockOption[className].push(btns)
            }
        }
        ...
        for (const tools of this.op.optionTools) {
            ...
            if (btns.length) this.btns.option.push(btns)
        }
        ...
        for (const tools of this.op.inlineTools) {
            ...
            for (const toolName in tools) {
                ...
                let { icon, name, description, menuMode, 
                    menuModeIcon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M18.71,8.21a1,1,0,0,0-1.42,0l-4.58,4.58a1,1,0,0,1-1.42,0L6.71,8.21a1,1,0,0,0-1.42,0,1,1,0,0,0,0,1.41l4.59,4.59a3,3,0,0,0,4.24,0l4.59-4.59A1,1,0,0,0,18.71,8.21Z"/></svg>',
                } = conf

                let btn: HTMLElement = document.createElement("div")
                btn.setAttribute("data-tool", toolName)
            ...
            if (btns.length) this.btns.inline.push(btns)
        }
    }
    
    initData(data: Record<string, any>, box: HTMLElement = this.blocksBox) {
        const { blocks } = data
        ...
        for (let key in blocks) {
            ...
            try {
                ...
                if (hasSetData) tool.setData(data)
                block = this.newBlock({
                    tool: toolName,
                    toFocus: false,
                    id: block.id,
                })
                if (hasSetData) tool.setData(null)
            ...
            for (let key in conf) {
                if (typeof this.tools.blockOption[toolName] !== "object") break
    ...
    initDesign() {
        ...
        this.cont.addEventListener("mouseout", (e) => {
            const node = (e.target as HTMLElement).closest("[data-elib-description]")
            if (!node) return
    ...
    initOther() {
        ...
        if (this.op.autofocus) {
            let blocks = this.cont.querySelectorAll('.elib-block') as NodeListOf<HTMLElement>
    ...
    newBlockMenu(block?: HTMLElement) {
        if (!block) {
            if (
                this.state.focusedBlock && 
        ...
        const search = (search: string, text: string) => {
        ...
        let focusedBtn: HTMLElement | Element | null = null
        const inpTrack = (e: Event) => {
            const target = e.target as HTMLElement
            let value = target.textContent
            const menu = this.cont.querySelector(".elib-newBlockMenu") as HTMLElement
            if (!menu) return
            if (!value) {
                target.innerHTML = ""
                unHide(menu)
            ...
            for (const sector of menu.children) {
                ...
                for (const btn of btns) {
                    btn.classList.remove("elib-focused")
                    let name = (btn.querySelector('.elib-toolBtns-name') as HTMLElement).textContent || ""
        ...
        const offNewblock = this.on('newBlockOnEnter', (e: AppEvent) => {
            e.prevent()
        }, { once: true })
        const offDeleting = this.on("delBlockOnBackspace", (e: AppEvent) => {
            e.prevent()
        })

        const keyTrack = (e: KeyboardEvent) => {
            if (e.key === "Enter") {
                e.preventDefault()
                unHide(menu)
        ...
        let oldDataInline: string
        
        const proceed = (block: HTMLElement | Boolean, clickOnTool = false) => {
            if (block instanceof HTMLElement) {
                inp = block.querySelector("[contenteditable]") as HTMLElement
                if (!inp) return
                ...
                if (!clickOnTool && inp === document.activeElement) inp.focus()
            }
        }
        ...
        menu.addEventListener('click', (event) => {
            ...
            if (!btn) return
            let tool = btn.getAttribute('data-tool') as string
            let callback = this.tools.block[tool].config.newBlock
            if (typeof callback === "function") {
                callback(tool, true)
                return
            }
            if (this.state.focusedBlock) this.deleteBlock(this.state.focusedBlock.block)
            const box = this.state.focusedBlock?.block.closest(".elib-blocks") as HTMLElement
        ...
        let offReplace = this.on("replaceBtns", (e: AppEvent) => {
            e.prevent()
        }, { last: true})
        
        let offFocus = this.on('focusBlocks', (e: AppEvent) => {
            e.prevent()
        }, { first: true })
        setTimeout(() => {
    ...
    async optionMenu() {
        if (!this.state.focusedBlock) return false
        const toolName = this.state.focusedBlock.block.getAttribute("data-tool") as string
        const optionBtn = this.btnsBox.children[1] as HTMLElement
        ...
        if (this.btns.blockOption[toolName]) {
            for (const btns of this.btns.blockOption[toolName]) {
                ...
                if (resBtns.length) options.push(resBtns)
            }
        }
        ...
        const unHide = (menu: HTMLElement) => {
            ...
            let btnBox = menu.querySelector('.elib-optionMenu-btnBox') as HTMLElement
        ...
        let focusedBtn: HTMLElement | null = null
        searchBar.addEventListener('input', (e) => {
            ...
            for (const sector of boxes) {
                ...
                for (const btn of btns) {
                    btn.classList.remove("elib-focusedSearchBtn")
                    let name = btn.querySelector('span')?.textContent as string
            ...
            if (emptyMenu) {
                ...
                if (span) span.remove()
                btnBox.classList.remove('elib-notFoundMode')
                let btns = btnBox.querySelectorAll(".elib-optionBtn") as NodeListOf<HTMLElement>
                for (let btn of btns) {
                    if (window.getComputedStyle(btn).display !== 'none') {
                        focusedBtn = btn
                        break
        ...
        const focusTo = (key: string) => {
            ...
            let next = btns[nextInx] as HTMLElement
    ...
    openInline(): HTMLElement {
        let menu = this.cont.querySelector(".elib-inlineMenu") as HTMLElement
        if (menu) return menu
        menu = document.createElement('div')
        menu.classList.add('elib-inlineMenu')

        const saveRange = (event: Event) => {
        ...
        menu.addEventListener("click", (e) => {
            ...
            if (!btn) return
            if (select.toString().length === 0) return
            
            let toolName = btn.getAttribute("data-tool") as string
    ...
    updateInlineContent(menu: HTMLElement | null) {
        if (!menu) return
    ...
    replaceInline(menu = this.cont.querySelector(".elib-inlineMenu") as HTMLElement) {
        if (!menu) return
        let select = window.getSelection() as Selection
        if (!select || !select.toString()) return
    ...
    isInline(inp?: HTMLElement): boolean {
        if (!inp) {
            let select = window.getSelection()
            if (!select || !select.toString()) return false
            let range = select.getRangeAt(0)

            let par = range.commonAncestorContainer as HTMLElement
            if (par.nodeName === '#text') {
                par = par.parentNode as HTMLElement
            }
            let block = par.closest('.elib-block')
            if (!block) return false

            let tool = block.getAttribute('data-tool')
            if (!this.allowed.inline.includes(tool)) return false
            
        
            inp = par.closest('[contenteditable="true"]') as HTMLElement
        }
        
        if (inp.getAttribute('data-elib-inline') === 'false') return false
        return true
    }
    ...
    newBlock(config: Record<string, any> = {}): HTMLElement | null {
        ...
        let block = document.createElement('div') as HTMLElement
        ...
        let rend: false | HTMLElement = false
        try {
        ...
        blockContent.addEventListener('keydown', (event) => {
            const target = event.target as HTMLElement
            if (target.closest(".elib-block") !== block) return
            if (target.closest("[data-elib-noProxy]")) return
    ...
    blockMeta(block: HTMLElement, callback?: (data: Record<string, any>) => any) {
        ...
        if (!data) {
            ...
            data = this.blocksMeta.get(block) as Record<string, any>
        }
    ...
    sanitize(html: string, tool: string) {
    ...
    async save(box = this.blocksBox): Promise<Record<string, any>> {
        const blocks = Array.from(box.children) as Array<HTMLElement>

        blocks.filter((block) => {
            if (block.classList.contains("elib-block")) return true
            return false
        })
        ...
        for (const block of blocks) {
            let toolName = block.getAttribute("data-tool") as string
            let config = (this.blocksMeta.get(block) as Record<string, any>)["config"]
            let options = (this.blocksMeta.get(block) as Record<string, any>)["option"]
            ...
            if (typeof tool.save === "function") {
                let res: false | HTMLElement = false
                try {
                    res = tool.save(block)
                } catch(e) {
            ...
            for (let key in config) {
                ...
                if (typeof plugin.save === "function") {
                    let res: false | Promise<any> | any = false
                    try {
    ...
    post() {
        const blocks = Array.from(this.blocksBox.querySelectorAll('.elib-block')) as Array<HTMLElement>
        ...
        for (const block of blocks) {
            ...
            const configs = (this.blocksMeta.get(block) as Record<string, any>)["config"]
            ...
            const options = (this.blocksMeta.get(block) as Record<string, any>)["option"]
    ...
    applyChange() {
        let blocks = this.cont.querySelector('.elib-blocks') as HTMLElement
    ...
    focusBlock(data: HTMLElement | Record<string, any>, option: Record<string, any> = {}) {
        ...
        if (!data.block.classList.contains("elib-block")) return
    ...
    deleteBlock(block: HTMLElement) {
        const box = block.closest(".elib-blocks") || this.blocksBox
        let newBlock = block.previousElementSibling || block.nextElementSibling as HTMLElement | null
        if (!newBlock) {
    ...
    focus(inp: HTMLElement) {
        ...
        if (inp instanceof HTMLInputElement || inp instanceof HTMLTextAreaElement) {
            ...
            if (document.contains(inp)) selection.addRange(range)
        }
    }

    create(component: "input", config: Record<string, any>, ...args: any[]): Element
    create(component: string, config: Record<string, any>, ...args: any[]): Element | null {
    ...
    getPosition(element: HTMLElement, cont = this.cont) {
    ...
    CET(cont: HTMLElement): HTMLElement | null
    CET(cont: HTMLElement, isNodelist: true): NodeListOf<HTMLElement>
    CET(cont: HTMLElement, isNodelist: boolean = false): NodeListOf<HTMLElement> | HTMLElement | null {
        const toNodelist = (el: HTMLElement): NodeListOf<HTMLElement> => {
            let tempCont = null
            ...
            let list = parent.querySelectorAll(`[${attr}=""]`) as NodeListOf<HTMLElement>
        ...
        list = cont.querySelectorAll('[contenteditable]') as NodeListOf<HTMLElement>
